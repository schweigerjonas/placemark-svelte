<script lang="ts">
	...
	import type { MarkerSpec } from "$lib/types/types";
...
<div class="card p-3" style="height: {height}vh">
	<div class="card-title flex items-center justify-between">
		<h5>{selectedMarker.spec.name}</h5>
		...
		<div class="flex grow flex-col items-start gap-3">
			<div class="flex items-center gap-1">
				...
				<div class="flex items-center py-1 text-xs">
					...
					<div class="rounded-r-lg bg-blue-400 pr-2 pl-1">{selectedMarker.spec.location.lat}</div>
				</div>
				<div class="flex items-center py-1 text-xs">
					<div class="rounded-l-lg bg-lime-200 pr-1 pl-2">Lng</div>
					<div class="rounded-r-lg bg-lime-400 pr-2 pl-1">{selectedMarker.spec.location.lng}</div>
				</div>
			</div>
			<div class="flex flex-col gap-1">
				<span class="font-bold">Information:</span>
				<p>{selectedMarker.spec.description}</p>
			</div>
		</div>
		<div class="w-100">
			<a href="/poi/{selectedMarker.spec._id}" class="btn btn-primary w-100 p-2">
